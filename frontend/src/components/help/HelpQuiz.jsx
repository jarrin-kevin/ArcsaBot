...
export const HelpQuiz = ({ onBackToChat }) => {
  ...
  const faqs = [
    ...
    {
      id: 3,
      icon: <KeyRound className="h-4 w-4 text-warn" />,
      ...
      <div className="space-y-3.5">
        {faqs.map((faq, index) => {
          ...
          return (
            <div
              key={faq.id}
              className="rounded-xl border border-line bg-surface-0 overflow-hidden transition duration-150"
            >
              <button
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-semibold text-xs leading-5 text-ink-900 md:text-sm">
                    {faq.question}
                  </span>
                </div>
                <span className="text-ink-500 ml-4 font-bold text-lg select-none">
                  {isOpen ? '−' : '+'}
                </span>
              </button>

              {isOpen && (
                <div className="px-4 pb-4 pt-1 text-xs leading-6 text-ink-700 border-t border-line whitespace-pre-line">
                  {faq.answer}
                </div>
              )}
            </div>
