...
## 8. Intervención post-evaluación: filtro de relevancia de citas

Tras cerrar la evaluación descrita en las secciones 1 a 7 (tag `tfe-evaluacion`, commit `25b9642`), se implementó en `chatbot/main.py` una función `_filter_citable_sources()` que corrige directamente el "mecanismo de citación fijo" señalado como causa del problema en la Sección 3: antes del fix, `chat()` citaba siempre las 5 fuentes recuperadas por Pinecone sin condicionarlas a ninguna medida de relevancia; con el fix, cada chunk debe superar individualmente `SIMILARITY_THRESHOLD = 0.70` (la misma constante que ya usaba `_is_low_confidence()`) para ser incluido en `sources`. Si ningún chunk del top-5 supera el umbral, la función devuelve una lista vacía en vez de forzar "el mejor de los 5" como fallback, evitando así presentar como evidencia validada un fragmento que el propio sistema ya trata como no confiable.

**Validación funcional del fix**: 46/46 tests pasan, el fix nunca cita más fuentes que el comportamiento anterior (es estrictamente un subconjunto de las 5 originales) y corrige de forma verificable los casos claramente fuera de cobertura semántica: en q036, el número de fuentes citadas bajó de 5 a 1, y el LLM generador respondió correctamente "no tengo información suficiente" en vez de citar evidencia tangencial.

**Por qué NO se re-corrió la evaluación completa de 40 casos con el fix aplicado**: esta no es una decisión de ahorro de costo sin respaldo técnico, sino una conclusión que ya se puede derivar de los datos existentes en `chatbot/eval/exactitud_cita.json` y `chatbot/eval/traces.jsonl`, sin necesidad de generar una sola llamada nueva al juez LLM:

- Cruzando los veredictos de criterio_2 (el fragmento respalda la afirmación) y criterio_5 (la disposición aplica al caso) contra las distancias reales registradas en `traces.jsonl`, las fuentes que ambos criterios marcaron como relevantes (criterio_2 Y criterio_5 = true, n=35) tienen una distance promedio de **0.7668** (rango 0.7250–0.8461). Las fuentes que el juez marcó como NO relevantes (n=85) tienen una distance promedio de **0.7584** (rango 0.7243–0.8203). La diferencia de medias es de solo **0.0084**, con un solapamiento casi total entre ambas distribuciones: no existe ningún valor de `SIMILARITY_THRESHOLD` que separe limpiamente las fuentes relevantes de las no relevantes en este corpus.
- Además, ninguno de los 32 casos en dominio del golden set tiene, en su top-5, un chunk con distance por debajo de 0.70. Es decir, el umbral actual (0.70) no descarta ninguna fuente en ninguno de esos 32 casos: el resultado de la Sección 3 (34.2% en criterio 2, 45.0% en criterio 5) no cambiaría al re-correr la evaluación con el fix aplicado, porque el conjunto de fuentes citadas sería idéntico al ya evaluado.
- Re-correr las Fases 3 y 4 completas implica volver a pagar llamadas reales a la API de OpenAI como juez: la corrida documentada en la Sección 7 (Fases 3 y 4 combinadas, con `gpt-5.5`) costó aproximadamente **$11 reales**, ya gastados. Gastar de nuevo esa cifra para confirmar empíricamente un resultado que el análisis de distancias de arriba ya demuestra que no cambia no está justificado: es dinero real gastado para no aprender nada nuevo.

Por ambas razones combinadas —la evidencia analítica ya disponible y el costo real y evitable de una confirmación redundante— se optó por no reejecutar el pipeline de evaluación completo. Esta decisión metodológica queda documentada aquí para que sea auditable y no se confunda con una omisión.

**Hallazgo técnico principal de esta intervención**: el fix es correcto y seguro (reduce sobre-citación en los casos donde la recuperación trae contenido fuera de dominio o con baja similitud global), pero **no resuelve el problema medido en la Sección 3**. Ese problema —fragmentos con similitud coseno relativamente alta que aun así no respaldan la afirmación (criterio 2) o no aplican al caso concreto (criterio 5)— es un problema de discriminación semántica fina dentro de la banda 0.72–0.85 de distance, una banda donde relevantes y no relevantes conviven casi sin separación. Ningún ajuste de umbral de similitud de embeddings puede resolver eso, porque el umbral opera sobre la misma señal (similitud vectorial) que ya no discrimina en ese rango. Se necesita un mecanismo que juzgue contenido real —por ejemplo, un reranker entrenado para relevancia fina, o un juez LLM que evalúe cada fragmento individualmente antes de decidir si se cita— en vez de un ajuste del threshold existente.

**Recomendación de costo para trabajo futuro con juez LLM**: si se retoma esta línea (por ejemplo, para evaluar un reranker o un juez de citación por fragmento), se recomienda usar `gpt-5.4-mini` en vez de `gpt-5.5` como modelo evaluador. Sobre los mismos volúmenes de tokens de la Fase 3 ya documentados en la Sección 7 (626,132 tokens de entrada + 150,532 de salida), el costo habría sido de aproximadamente **$1.15** con `gpt-5.4-mini` frente a los **≈$7.65** reales pagados con `gpt-5.5` (precios verificados: `gpt-5.5` $5/$30 por millón de tokens de entrada/salida, `gpt-5.4-mini` $0.75/$4.50 por millón).
